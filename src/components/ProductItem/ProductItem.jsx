...
export default function ProductItem({ product, loading, cruntid, addToCart, AddWishList, removewashList }) {
  ...
  return (
    ...
    >
      ...
      <div className="product py-2 relative border-transparent rounded-xl border-4 hover:border-indigo-400 hover:bg-indigo-50 transition-all duration-700 p-2">
        {/* add to wishList */}
        ...
        <figure className="relative overflow-hidden">
          ...
          <figcaption className="mt-2 ">
            <p className="text-indigo-600">{product.category.name}</p>
            ...
            <div className="flex flex-col gap-2 justify-center items-center transition-transform transform group-hover:translate-y-0 translate-y-full">
              <button disabled={loading && product.id == cruntid} onClick={() => addToCart(product.id)} className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg transition-transform transform">
              ...
              <Link to={`/freshcart/productdetails/${product.id}`}>
                <button className="bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg transition-transform transform">
